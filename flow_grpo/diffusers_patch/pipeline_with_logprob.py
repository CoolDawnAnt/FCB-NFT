...
def pipeline_with_logprob(
    ...
    # 5. Prepare timesteps
    if not flux:
        # custom_sigmas: few-step EVEN grid (e.g. [1,0.75,0.5,0.25]) passed in by the caller so
        # train and inference share the SAME schedule; the scheduler still applies its static shift.
        # None => diffusers default linspace(1, ~0) (last query lands at ~0.001 -- not what we want).
        timesteps, num_inference_steps = retrieve_timesteps(
            self.scheduler,
            num_inference_steps if custom_sigmas is None else None,
            device,
